...
namespace DAL
{
    public class mp_pago_deposito
    {
        ...
        {
            List<BE.Pago_Deposito> pagos = new List<BE.Pago_Deposito>();
            List<BE.Factura> facturas = factura.Listar();
            ...
            ac.Abrir();
            DataTable tabla = ac.Leer("select * from pago_deposito");
            ac.Cerrar();

            foreach (DataRow registro in tabla.Rows)
            {
                BE.Pago_Deposito p = new BE.Pago_Deposito();
                ...
                p.Factura = (from Factura f in facturas
        ...
        public int Insertar(BE.Pago_Deposito pago)
        {
            string sql = "Select isnull(max(id),0) + 1 from pago_deposito";
            ac.Abrir();
            pago.Id = ac.DevolverEscalar(sql);

            sql = "Insert into pago_deposito (id, id_tipo_tarjeta, id_procesador_pago, id_banco, numero, codigo, vencimiento," +
                  "id_factura, cuotas) values (@i, @it, @ip, @ib, @n, @c, @v, @if, @cu)";
            ...
            parametros.Add(ac.CrearParametro("@if", pago.Factura.Id));
